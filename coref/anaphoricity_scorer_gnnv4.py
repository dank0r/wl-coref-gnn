...
class Net(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, edge_dim=60, dropout_rate=0.3, n_ants=50):
        super().__init__()
        self.n_ants = n_ants
        self.conv1 = TransformerConv(in_channels, hidden_channels, edge_dim=edge_dim, heads=1)
        self.conv2 = TransformerConv(hidden_channels, hidden_channels, edge_dim=edge_dim, heads=1)
        self.conv3 = TransformerConv(hidden_channels, hidden_channels, edge_dim=edge_dim, heads=1)
        # self.conv4 = GATv2Conv(hidden_channels, out_channels, edge_dim=edge_dim)

        net_hidden_channels = 1024
    ...
    def forward(self, x, edge_index, edge_features, batch_size, n_mentions):
        z = self.encode(x, edge_index, edge_features)
        res = self.decode(z[:-1], edge_index[:, :-n_mentions*2], edge_features[:-n_mentions*2])
...
class AnaphoricityScorer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # layers = []
        self.gnn = Net(config.hidden_size, 256, 256, 60, dropout_rate=config.dropout_rate, n_ants=config.rough_k)
    ...
    def forward(self, *,  # type: ignore  # pylint: disable=arguments-differ  #35566 in pytorch
        ...
        n_mentions = all_mentions.size(0)
        # [batch_size, n_ants, pair_emb]
        # pair_matrix = self._get_pair_matrix(
        #    all_mentions, mentions_batch, pw_batch, top_indices_batch)
        # [n_nodes, mention_emb]
        x = all_mentions
        master_node = torch.ones((1, all_mentions.size(1)), device=device)
        x = torch.cat([x, master_node], dim=0)
        ...
        edges2master = torch.cat([
            torch.unsqueeze(torch.arange(n_mentions, device=device), dim=0),
            torch.full((1, n_mentions), n_mentions, device=device)
        ], dim=0).long()
        edges2master = torch.cat([edges2master, torch.flip(edges2master, dims=[0])], dim=1)
        edge_index = torch.cat([edge_index, edges2master], dim=1)


        # [batch_size * n_ants, 60]
        pw_batch_tmp = pw_batch.reshape(-1, pw_batch.size(-1))
        # [2 * batch_size * n_ants, 60]
        edge_features = torch.cat([
            pw_batch_tmp,
            pw_batch_tmp,
            torch.zeros((edges2master.size(1), 60), device=device)
        ], dim=0)


        # [batch_size, n_ants]
        scores = top_rough_scores_batch + self.gnn(x, edge_index, edge_features, batch_size, n_mentions)
